...
void mouseReleased () {
    ...
    */
    if (shape != 0) {
        ...
        */
        shape_tracker.add(new ShapeTracker(lines.size()-1, shape));
        System.out.println("Line: " + (lines.size()-1) + " Shape: " + shape_tracker.get(shape_tracker.size()-1).shape);    /* ***************** SHAPE TRACKER for other than free lines ******************** */
        
        for (int i=straight_line_points.size()-1; i>=0; i--) {
            straight_line_points.remove(i);
        }
        //System.out.println("Total SLPs: " + straight_line_points.size());       
    }
...
void draw() {
    ...
    */
    ...
    stroke(line_color); //black=0, red=FF0000, green=00FF00, blue=0000FF
    
    /* Draw instant free-form lines and adds traker
    */
    if(mousePressed && shape == 0) {
        lines.add(new Line (pmouseX, pmouseY, mouseX, mouseY));
        shape_tracker.add(new ShapeTracker(lines.size()-1, shape));    /* ***************** SHAPE TRACKER for free lines ******************** */
